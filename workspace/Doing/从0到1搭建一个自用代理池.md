...
这里有个环境细节：服务器上 Redis 是 6.0，没有 `HRANDFIELD` 这个命令（6.2 才有），所以取随机代理只能用 `HKEYS` 拉出来自己 rand。这个限制在后面直接决定了取用逻辑的写法。
...
抓回来的候选不能直接用，里面混着死代理、格式错的、还有假装是代理的。MVP 阶段只做两件事，算第一刀：格式校验，加存活验证。
...
class ProxyService:
    def __init__(self, fetcher=None, checker=None, pool=None):
        self.fetcher = fetcher or TheSpeedXHttpFetcher()
        self.checker = checker or Checker()
        self.pool = pool or RedisPool()

    def refresh(self):
        """抓新代理 → 验证 → 首次入库（不重复放）。"""
        proxies = list(self.fetcher.fetch())
